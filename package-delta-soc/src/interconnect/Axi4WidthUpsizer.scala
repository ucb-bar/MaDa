...
class Axi4WidthUpsizer(
  ...
  val aw_address_offset = io.s_axi.aw.bits.addr(m_addr_index-1, s_addr_index)
  val ar_address_offset = io.s_axi.ar.bits.addr(m_addr_index-1, s_addr_index)
  ...
  io.m_axi.w.bits.data := Mux(
    io.s_axi.aw.fire,
    io.s_axi.w.bits.data << (s_params.dataWidth.U * aw_address_offset),
    io.s_axi.w.bits.data << (s_params.dataWidth.U * reg_aw_address_offset)
  )
  io.m_axi.w.bits.strb := Mux(
    io.s_axi.aw.fire,
    io.s_axi.w.bits.strb << ((s_params.dataWidth.U / 8.U) * aw_address_offset),
    io.s_axi.w.bits.strb << ((s_params.dataWidth.U / 8.U) * reg_aw_address_offset)
  )
  io.s_axi.r.bits.data := Mux(
    io.m_axi.ar.fire,
    io.m_axi.r.bits.data >> (s_params.dataWidth.U * ar_address_offset),
    io.m_axi.r.bits.data >> (s_params.dataWidth.U * reg_ar_address_offset)
  )
}